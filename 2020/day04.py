input_passports="""iyr:2010 ecl:gry hgt:181cm
...
eyr:2027 iyr:2019 ecl:#a0eeca hcl:#c0946f pid:724783488 byr:1943 cid:282
hgt:124
...
test_passport="""ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
only_valid="""pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
for passport in input_passports.split('\n\n'):
    passdict = {line.split(':')[0]:line.split(':')[1] for line in re.split(' |\n', passport)}
    passports.append(passdict)
    #for line in passport.split(' '):
        # print("newline")
        # print(line.split(':')[0]+" bla "+ line.split(':')[1])

valid = 0
haircolor = re.compile('#[0-9a-f]{6,6}')
passid = re.compile('[0-9]{9,9}$')
eyecolor = ['amb', 'blu', 'brn', 'gry', 'grn','hzl','oth']
for passport in passports:
    if 'cid' in passport:
        del passport['cid']
    result = sorted(list(passport.keys())) == sorted(fields)
    if not result:
        continue
    ...
    if not ( 2020 <= int(passport.get('eyr')) <= 2030):
        continue
    if haircolor.match(passport.get('hcl')) == None:
        continue
    if passport.get('ecl') not in eyecolor:
        continue
    pid = passport.get('pid')
    if passid.match(pid) == None:
        continue
