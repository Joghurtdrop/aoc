customs="""rypdeiqkbgacnxwumhtozfjvs
...
for line in customs.splitlines():
    if len(line) == 0:
        sum_of_customs += len(set_of_answers)
        set_of_answers.clear()
        new_line = True
        continue

    if new_line:
        set_of_answers = set(line)
        new_line = False

    set_of_answers = set_of_answers & set(line)
    print(set_of_answers)

sum_of_customs += len(set_of_answers)
print(sum_of_customs)
